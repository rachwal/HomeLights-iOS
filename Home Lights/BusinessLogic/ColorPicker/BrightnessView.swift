...
class BrightnessView: UIView {
    ...
    init(frame: CGRect, color: UIColor) {
        ...
        colorLayer.frame = CGRect(x: 0, y: 2, width: self.frame.size.width, height: 48)
    ...
    func drawIndicator() {
        // Draw the indicator
        if (point != nil) {
            indicator.path = UIBezierPath(roundedRect: CGRect(x: point.x-3, y: 0, width: 6, height: 52), cornerRadius: 3).CGPath
        }
    }
